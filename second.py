...
def tapPay(
    number, prime, attName, attDate, attTime, price, conPhone, userName, attEmail
):
    ...
    myobj = {
        ...
        "amount": price,
        "cardholder": {"phone_number": conPhone, "name": userName, "email": attEmail},
    }
...
def orderResponse(
    cursor, orderNumber, attEmail, attId, attDate, attTime, price, name, phone, status
):
    global orderResp
    ...
    orderResp = {
        ...
        "contact": {"name": name, "email": attEmail, "phone": phone},
        "status": status,
    }
...
def order_post():
    try:
        ...
        attAddress, attImage = (
            request.get_json()["order"]["trip"]["attraction"]["address"],
            request.get_json()["order"]["trip"]["attraction"]["image"],
        )
        attDate, attTime = (
        ...
        if request.method == "POST":  # 取得當前登入的使用者資訊
            ...
            if decrypt is None:
                ...
                if (
                    sqlA is None
                    or sqlA[1] != attId
                    or sqlA[2].strftime("%Y-%m-%d") != attDate
                    or sqlA[3] != attTime
                    or sqlA[4] != price
    ...
    try:
        ...
        if request.method == "DELETE":  # 刪除預定行程
            ...
            if decrypt is None:
                ...
                if one is not None:  # 有預訂過
                    delete = (
                        """ delete from `member_bookrecord` where PersonEmail = %s """
                    )
                    cursor.execute(delete, (decrypt["email"],))
        ...
        if request.method == "PATCH":  # 登入一個使用者
            ...
            if one is None:
                ...
                access_token = create_access_token(
                    identity={"email": email_f1}, expires_delta=timedelta(seconds=1800)
                )
                print(access_token)
...
def page_keyword():
    try:
        ...
        if keyword is None:
            ...
            if (
                _12site == [] or _12site[-1][0] == total
            ):  # 若12景點為空 或是最後景點的id為total，表示已無下一頁
                nextpage = None
            else:
                nextpage = page + 1
            ...
            if (
                _12site == [] or _12site[-1][0] == lastKey_i
            ):  # 若12景點為空 或是最後景點的id為lastKey_i，表示已無下一頁
                nextpage = None
            else:
                nextpage = page + 1

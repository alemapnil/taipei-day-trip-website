function makeDiv(name, cate, descp, address, transport, mrt, images) {
  ...
  document
    .getElementsByClassName("mrt_cat")[0]
    .appendChild(document.createTextNode(`${cate} at ${mrt}`));
  document
    .getElementsByClassName("des")[0]
    .appendChild(document.createTextNode(descp));
  document
    .getElementsByClassName("addr_des")[0]
    .appendChild(document.createTextNode(address));
  document
    .getElementsByClassName("traf_des")[0]
...
function greenBtn() {
  ...
  let price = parseInt(
    document.getElementById("dollar").innerText.replace(",", "")
  ); //價格

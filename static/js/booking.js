...
async function bookGet() {
  ...
  document.querySelector(
    ".Bheader"
  ).innerHTML = `您好，${userName}，待預訂的行程如下：`;
  ...
  if (dict["data"] === false) {
    ...
    document.getElementById("load").remove();
    document.querySelector(".whole").style.display = "block";
    (attId = dict["data"]["attraction"]["id"]),
      (attName = dict["data"]["attraction"]["name"]);
    (attAddress = dict["data"]["attraction"]["address"]),
      (attImage = dict["data"]["attraction"]["image"]);
    (attDate = dict["data"]["date"]),
      (attPrice = dict["data"]["price"]),
      (attTime = dict["data"]["time"]);
    ...
    let trash =
      '<div class="cancel"><img src="/static/img/icon_delete.jpg"></div>';
    document.querySelector(
      ".item_t"
    ).innerHTML = `台北一日遊：${attName}${trash}`;

    whole.querySelector(".cancel").addEventListener("click", cancel);

    for (let i = 0; i < whole.querySelectorAll(".item").length; i++) {
      if (i === 0) {
        whole.querySelectorAll(".item")[
          i
        ].innerHTML = `<span>日期：</span>${attDate}`;
      } else if (i === 1) {
        whole.querySelectorAll(".item")[
          i
        ].innerHTML = `<span>時間：</span>${attTime}`;
      } else if (i === 2) {
        whole.querySelectorAll(".item")[
          i
        ].innerHTML = `<span>費用：</span>新台幣 ${attPrice} 元`;
      } else if (i === 3) {
        whole.querySelectorAll(".item")[
          i
        ].innerHTML = `<span>地點：</span>${attAddress}`;
      }
    }
...
TPDirect.setupSDK(
  124024,
  "app_8PZWHViTnDMvcfP62JynEsRUVkoXu2Wf5R7tneWZtDtGJLjDbKkzFqka66fM",
  "sandbox"
);
...
function clickPay(event) {
  ...
  if (
    !(
      conName.match(pat_name) &&
      conEmail.match(pat_email) &&
      conPhone.match(/^09\d{8}$/)
    )
  ) {
    alert("您的聯絡資訊格式不正確");
    return;
  ...
  fetch("/api/orders", {
    ...
    .then(function (dict) {
      if ("message" in dict) {
        ...
        let number = dict["data"]["number"],
          msg = dict["data"]["payment"]["message"];
